...
- **CSP & headers:** Meta CSP in `BaseLayout` includes **`unsafe-eval`** and **`*.beatstars.net` / `*.beatstars.com`** in `script-src` and `connect-src` so the Beatstars embed can run; `frame-src` allows **`https://www.beatstars.com`**, wildcards on `*.beatstars.net`, and **`https://www.youtube.com`** for embeds. GTM/GA/GoatCounter remain on `script-src` / `connect-src` alongside `base-uri`, `form-action`, and `object-src 'none'`. **`public/_headers`** still adds transport headers on supporting hosts.
